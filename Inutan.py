...
header = Frame(root, height=100, width=500, bg="#d1cfe2")
header.propagate(0)
header.grid(row=0, column=0)
Label(header, text="STUDENT INFORMATION SYSTEM", font = 'Helvitica 20 bold', bg="#d1cfe2", fg="white").place(relx=.5, rely=.4, anchor='c')
Label(header, text="a simple demonstration", font = 'Helvitica 12 italic', bg="#d1cfe2", fg="white").place(relx=.5, rely=.65, anchor='c')

#Code for a dynamic searchbar
searchFrame = Frame(root)
searchFrame.grid(row=1,column=0,pady=3)
Label(searchFrame, bg="#d1cfe2", text="SEARCH:", anchor=W, font = 'Arial 9 bold', fg="white").grid(row=0, column=0)
...
myFrame= Frame(mycanvas, bg="#9cadce")
...
add = Button(root,image=button_addstudent, borderwidth=0,bg='#d1cfe2', command=lambda:info_window("add",1))
